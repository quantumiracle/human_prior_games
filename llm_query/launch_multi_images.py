    ...
    while True:    
        ...
        with open(input_fifo_name, 'rb') as f:
            ...
            if len(splitted_msg) == 2:  # only one image
                ...
                # image.show()
            else:  # more than one image, return list of images
                image = []
                for img_str in splitted_msg[1:]:
                    byte_io = io.BytesIO(img_str)
        ...
        for _ in range(NUM_QUERIES):
            ...
            if image is not None:
                # first message
                if IMAGE_PLACEHOLDER in inp: 
                    if model.config.mm_use_im_start_end:
                        inp = re.sub(IMAGE_PLACEHOLDER, image_token_se, inp)
                    else:
                        inp = re.sub(IMAGE_PLACEHOLDER, DEFAULT_IMAGE_TOKEN, inp)
                else:
                    if model.config.mm_use_im_start_end:
                        inp = image_token_se + '\n' + inp
                    else:
