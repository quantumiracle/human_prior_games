...
class replay_buffer:
    ...
    def add(self, samples):
        ...
        wrap_tensor = lambda x: torch.tensor(np.ndarray([x]))
...
def rollout(env, model, run_name):
    ...
    for epi in range(MAX_EPI):
        ...
        if qs is not None:
            writer.add_scalar(f"charts/q_mean", torch.mean(qs), epi)
            writer.add_scalar(f"charts/q_max", torch.max(qs), epi)
            writer.add_scalar(f"charts/q_min", torch.min(qs), epi)
