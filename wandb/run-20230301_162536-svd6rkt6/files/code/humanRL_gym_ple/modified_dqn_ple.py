...
class traj_buffer:
    ...
    def add(self, traj, epi_r):
        if len(self.traj_buffer) < self.buffer_size:
            self.traj_buffer.append(traj)
            self.r_buffer.append(epi_r)
        else:
            self.traj_buffer[self.location] = traj
    ...
    def sample(self,):
        max_r_id = np.argmax(self.r_buffer)[0]
        return self.traj_buffer[max_r_id]
...
def rollout(env, model, run_name):
    ...
    traj_buffer = traj_buffer(TRAJ_BUFFER_SIZE)
    log = []
    qs = None
    ...
    for epi in range(MAX_EPI):
        ...
        a_list = []
        if len(traj_buffer.buffer) >= 1 and np.random.uniform() > eps:   # greedy
            greedy = True
            a_seq = traj_buffer.sample()
        else:
            greedy = False
        ...
        for step in range(MAX_STEP):
            ...
            if done:
                break
            s = s_
        traj_buffer.append(a_list, epi_r)
...
if __name__ == '__main__':
    ...
    env_name = 'smallGame'
    env = ImageToPyTorch(PLEEnv(env_name, downsample_size=(14, 14)))
