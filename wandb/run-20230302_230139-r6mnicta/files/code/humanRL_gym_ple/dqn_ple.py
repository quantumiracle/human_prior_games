...
class QNetworkCNN(nn.Module):
    def __init__(self, num_actions, in_shape, out_channels=3, kernel_size=3, stride=1, hidden_units=32):
        ...
        self.conv = nn.Sequential(
            nn.Conv2d(in_channels, int(out_channels), kernel_size, stride),
            nn.LeakyReLU(),
            # nn.MaxPool2d(kernel_size, stride=2),
            nn.Conv2d(int(out_channels), int(out_channels), kernel_size, stride),
            nn.LeakyReLU(),
            # nn.MaxPool2d(kernel_size, stride=2)
        )
...
class PrioritizedReplayBuffer:
    def __init__(self, buffer_size, alpha=0.6, beta=0.4, beta_annealing=0.99995):
    ...
    def add(self, samples):
        ...
        self.priorities[self.location] = max_priority
    ...
    def sample(self, batch_size):
        if len(self.buffer) == self.capacity:
            ...
            priorities = self.priorities[:self.location]
...
class DQN(object):
    def __init__(self, env):
        ...
        self.updates = 0
        self.replay_buffer = PrioritizedReplayBuffer(REPLAY_BUFFER_SIZE) if PER else ReplayBuffer(REPLAY_BUFFER_SIZE)
    ...
    def choose_action(self, x, greedy=False):
        ...
        if greedy or np.random.uniform() > epsilon:   # greedy
    ...
    def learn(self):
        if PER:
            indices, sample, weights = self.replay_buffer.sample(BATCH_SIZE)
        ...
        # Update with loss
        if PER:
            td_errors = target.detach().numpy() - Q_s_a.detach().numpy()
            td_errors = td_errors.squeeze(1)
            td_errors = torch.FloatTensor(td_errors)
            priorities = np.abs(td_errors) + 1e-6
            self.replay_buffer.update_priorities(indices, priorities)
            # loss = torch.mean(weights * self.loss_func(Q_s_a, target.detach()))
            loss = torch.mean(weights * f.smooth_l1_loss(target.detach(), Q_s_a))


        else:
            # loss = torch.mean(self.loss_func(Q_s_a, target.detach()))
...
def eval(env, model, eval_episodes=1):
    epi_r = 0
    for epi in range(eval_episodes):
        s=env.reset()
        for step in range(MAX_STEP):
            env.render()
            a, _ = model.choose_action(s, greedy=True)
            s_, r, done, info = env.step(a)
            epi_r += r
            s = s_
            if done:
                break
    return epi_r/float(eval_episodes)
