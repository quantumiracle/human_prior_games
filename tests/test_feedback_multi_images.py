...
def get_llm_feedback(image, last_image=None):
    image = Image.fromarray(image)
    if last_image is not None:
        last_image = Image.fromarray(last_image)
    # question = "USER: what is the difference of current frame and last frame? \n Last frame: <image-placeholder> \n Current frame: <image-placeholder>?\nASSISTANT: "
    question = "USER: are the two images different? \n Last frame: <image-placeholder> \n Current frame: <image-placeholder>?\nASSISTANT: "

    # question = "Last frame: <image-placeholder> \n\
    #     Current frame: <image-placeholder> \n\
    #     Context: This is a frame of a video game. The agent to control is a white and gray character, \
    ...
    images = [last_image, image]
    answer = query_llm(images, question)
    ...
    for _ in range(1):
        ...
        for i in range(1000):                
            if (i-1) % query_freq == 0:  # skip first black frame
                ...
                answer = get_llm_feedback(frame, last_frame)
                last_frame = frame
                try:
